...
class QuizLibrary extends _$QuizLibrary {
  ...
  Future<List<LibraryItem>> build() async {
    AppLogger.debug('[LIBRARY_PROVIDER] build() called - starting data fetch');

    // Use Firebase Auth directly as the source of truth for user state
    final user = FirebaseAuth.instance.currentUser;

    AppLogger.debug('[LIBRARY_PROVIDER] Firebase user: ${user?.uid}');
    ...
    if (user != null) {
      ...
      try {
        ...
        AppLogger.error('[LIBRARY_PROVIDER] ERROR in build(): $e');
        AppLogger.error('[LIBRARY_PROVIDER] Stack trace: $stackTrace');
        rethrow;
      }
    } else {
  ...
  Future<void> _fetchUsernames(List<LibraryItem> items) async {
    ...
    try {
      ...
      AppLogger.error('Error batch fetching usernames: $e');
  ...
  Future<void> _markFavorites(List<LibraryItem> items) async {
    ...
    try {
      ...
      AppLogger.debug(
        '[LIBRARY_PROVIDER] Found ${favoriteIds.length} favorites',
      );
      ...
      AppLogger.error('Error marking favorites: $e');
  ...
  Future<void> reload() async {
    AppLogger.debug('[LIBRARY_PROVIDER] Reloading library...');
    state = const AsyncValue.loading();
    state = await AsyncValue.guard(() => build());
    AppLogger.success('[LIBRARY_PROVIDER] Reload completed');
  }
